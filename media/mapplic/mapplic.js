	...
	var Mapplic = function(element) {
		...
		function Sidebar() {
			...
			this.addCategories = function(categories) {
				 ...
				 if (categories) {
					$.each(categories, function(index, category) {
						...
						$('<span></span>').text('0').addClass('mapplic-list-count parentcount').prependTo(link);
						list.append(item);
					});
				}  
			}		
			this.addSubLocation = function(data) {
						var subitem =	$('<li></li>').addClass('mapplic-list-category').addClass('mapplic-sublist-shown').attr('data-subcategory', data.id);
						var subol = $('<ol></ol>').css('border-color', 'red').appendTo(subitem);
						subol.hide();					
						var sublink = $('<a></a>').attr('href', '#').attr('title', data.title).css('background-color', data.color).text(data.title).prependTo(subitem);
						sublink.on('click', function(e) {
							e.preventDefault();
							subitem.toggleClass('mapplic-opened');
							subol.slideToggle(200);
						});	
						$('<span></span>').text('0').addClass('mapplic-list-count subcount').prependTo(sublink); 
						var category = $('.mapplic-list-category[data-category="' + data.category + '"]');
				 $('.mapplic-list-category[data-category="' + data.category + '"]').find('ol:first').append(subitem);	
				$('.parentcount', category).text($('.mapplic-sublist-shown', category).length);
			...
			this.addLocation = function(data) {
				...
				var subcategory = $('.mapplic-list-category[data-subcategory="' + data.subcategory + '"]');
				if (subcategory.length) $('ol', subcategory).append(item);

				// Count
				$('.parentcount', category).text($('.mapplic-sublist-shown', category).length);
				$('.mapplic-list-count', subcategory).text($('.mapplic-list-shown', subcategory).length);
			}
...
/* 				$('.mapplic-list .mapplic-sublist-shown ol li', self.el).each(function() {
					...
					$('.mapplic-list li', self.el).each(function(e) {					if($(this).find("ol").length == 0)					{					}					else					{
					if ($(this).text().search(new RegExp(keyword, "i")) < 0) {						
						$(this).removeClass('mapplic-list-shown');
						$(this).slideUp(200);
					} else {
				...
				$('.mapplic-list > li ', self.el).each(function() {
					var count = $('.mapplic-sublist-shown.mapplic-list-shown', this).length;
					$('.parentcount', this).text(count);
				});
				$('.mapplic-list > li ol li', self.el).each(function() {
		...
		var processData = function(data) {
			...
			if (data.subcategories) {
				$.each(data.subcategories, function(index, subcategory) {
					if (self.sidebar) self.sidebar.addSubLocation(subcategory);
				});
			}
